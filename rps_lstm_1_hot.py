...
#     [
#         keras.Input(shape=(look_back, input_dim)),
#         layers.Dropout(dropout_pct),
#         layers.Bidirectional(layers.SimpleRNN(hiddenUnits, return_sequences=True)),
#         layers.Dropout(dropout_pct),
#         layers.Bidirectional(layers.SimpleRNN(hiddenUnits)),
...
model = keras.Sequential(
    [
        keras.Input(shape=(look_back, input_dim)),
        layers.Dropout(dropout_pct),
        layers.Bidirectional(layers.LSTM(num_nodes, return_sequences=True)),
        layers.Dropout(dropout_pct),
        layers.Bidirectional(layers.LSTM(num_nodes)),
        layers.BatchNormalization(),
        layers.Dense(output_dim, activation="softmax")
    ]
)
...
for index, predict in enumerate(PredictResult):
    ...
    AI_move = (predict_index + 1) % 3  # calculate counter move
    if predict_index == truth[index].argmax(axis=0):  # if prediction is correct
        AI_win.append(1)  # win
    elif AI_move == truth[index].argmax(axis=0):  # if counter move same as opponent
        AI_win.append(0)  # tie
    else:
